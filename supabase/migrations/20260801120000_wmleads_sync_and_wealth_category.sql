-- Wealth Management category + WmLeads sync cron (parallel to Base44 SaLead / Funnel).

INSERT INTO public.categories (name, slug)
VALUES ('Wealth Management', 'wealth-management')
...
BEGIN
  ...
  PERFORM cron.schedule(
    ...
    format(
      ...
      $cmd$,
      v_base_url || '/api/cron/wmleads-sync',
      jsonb_build_object('x-cron-secret', v_secret)::text
    )
  );
...
COMMENT ON COLUMN public.external_sync_cursors.ingest_from IS
  'Do not import leads from Base44/Funnel/WmLeads with source created/updated timestamps before this instant.';
