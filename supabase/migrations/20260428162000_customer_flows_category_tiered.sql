-- Remove package dependency from customer lead flows.
-- Flows now route by (organization_id, category_id, unit_type).
...
ALTER TABLE public.customer_lead_flows
  ADD COLUMN IF NOT EXISTS unit_type public.lead_unit_type NOT NULL DEFAULT 'single';

UPDATE public.customer_lead_flows f
SET category_id = lp.category_id
FROM public.lead_packages lp
WHERE f.package_id = lp.id
...
ALTER TABLE public.customer_lead_flows
  DROP CONSTRAINT IF EXISTS customer_lead_flows_organization_id_package_id_key;

CREATE UNIQUE INDEX IF NOT EXISTS uq_customer_lead_flows_org_category_unit
  ON public.customer_lead_flows (organization_id, category_id, unit_type);
