-- Cap free leads delivery per organization with fair proportional distribution
-- across orgs until each reaches quota_total.
...
-- Orgs with delivered free leads but no allocation row yet.
UPDATE
  public.organization_free_delivery d
SET
  quota_total = GREATEST(d.quota_delivered, 1)
WHERE
  d.quota_total = 0
  AND d.quota_delivered > 0;

ALTER TABLE public.organization_free_delivery
  DROP CONSTRAINT IF EXISTS organization_free_delivery_quota_range;

ALTER TABLE public.organization_free_delivery
  ADD CONSTRAINT organization_free_delivery_quota_range CHECK (
    quota_total >= 0
...
BEGIN
  ...
  IF v_delivery.quota_total <= 0 THEN
    RAISE EXCEPTION 'Free delivery quota is not configured for this organization';
  END IF;
  IF v_delivery.quota_delivered >= v_delivery.quota_total THEN
    RAISE EXCEPTION 'Free delivery quota exhausted';
  END IF;
