-- Replace quota-based free test leads with equal-split free leads delivery.
-- Paying customers are served first via run_due_customer_lead_flows; remaining
-- inventory is distributed equally across orgs with free delivery enabled.
...
-- Widen constraint before renaming grant_source values.
...
-- Final constraint: free_test removed after data migration.
ALTER TABLE public.customer_leads
  DROP CONSTRAINT IF EXISTS customer_leads_funding_mode_check;
...
-- Equal split: fewest free_delivery leads first, then earliest activation.
