-- Only inventory created on/after eligible_from (campaign start day) counts for free delivery.
-- Supports starting a fresh campaign after a prior one completes.
...
SET
  eligible_from = date_trunc(
    'day',
    (COALESCE(d.activated_at, d.created_at, now()) AT TIME ZONE 'UTC')
  ) AT TIME ZONE 'UTC'
WHERE
  d.eligible_from IS NULL;

ALTER TABLE public.organization_free_delivery
  ALTER COLUMN eligible_from SET DEFAULT (date_trunc('day', (now() AT TIME ZONE 'UTC')) AT TIME ZONE 'UTC');
